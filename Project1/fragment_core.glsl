...
void main()
{
	...
	//* (vec4(ambientFinal, 10.f) + vec4(diffuseFinal, 1.f) + vec4(specularFinal, 1.f));

	//without specular
	fs_color = 
	texture(material.diffuseTex, vs_texcoord)
	* (vec4(ambientFinal * 10, 1.f) + vec4(diffuseFinal, 1.f) + vec4(specularFinal, 1.f));
}